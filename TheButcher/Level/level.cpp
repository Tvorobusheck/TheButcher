...
MonsterObject* Level::addMonster(int x, int y,
    ...
    MonsterObject* newMonster = new MonsterObject(x, y, amountOfMaxHealth,
                                                  weaponDamage,
...
void Level::takeMove(MapObject* character, Level::Point 
                     vectorCoordinates){
...
bool Level::heroAbilityToBuyOnePotion(){
    return trader -> getTrader() -> abilityToPayPotion(mainHero ->
                                                       getHeroState());
}

void Level::heroBuysPotion(){
    return trader -> getTrader() -> heroBuysPotion(mainHero ->
                                                   getHeroState());
}
void Level::heroBuysNewWeapon(WeaponTypes weaponType){
    ...
    trader -> getTrader() -> heroBuysWeapon(mainHero -> getHeroState(),
                                            weaponLibType);
}
...
void Level::monsterAttaksHero(MonsterObject* monster){
    if(getMonsterWeaponType(monster))
        mainHero -> getHeroState() -> takeDamage(monster -> getMonsterState() ->
                                                 getDamage());
    else
        shots.push_back(new Shot(this, monster));
}
void Level::heroGetsMonsterDrop(MonsterObject* monster){
    mainHero -> getHeroState() -> takeGold(monster -> getMonsterState() ->
                                           getGoldDrop());
    mainHero -> getHeroState() -> takeExp(monster -> getMonsterState() ->
                                          getExpDrop());
}
...
Level::WeaponTypes Level::getCurrentWeaponType(){switch (mainHero -> 
                                                         getHeroState() -> getWeaponType()) {
    case Weapon::TypeOfWeapon::Gun:
...
void Level::checkArea(unsigned int radius){
    for(auto it: monsters)
        if(1ull * (it -> getPosX() - mainHero -> getPosX())* (it -> getPosX() -
                                                              mainHero -> getPosX()) +
                1ull*(it -> getPosY() - mainHero -> getPosY())* (it -> getPosY() -
                                                                 mainHero -> getPosY()) <= radius * radius)
...
void Level::combatMonstersAtackHero(){
    for(auto it: monsters)
        if(it -> isAttacks()){
            if(it -> getMonsterState() -> getWeaponType() ==
                    Weapon::TypeOfWeapon::Melee){
                if(1ull * (it -> getPosX() - mainHero -> getPosX())* (it -> getPosX() -
                                                                      mainHero -> getPosX()) +
                        1ull*(it -> getPosY() - mainHero -> getPosY())* (it -> getPosY() -
                                                                         mainHero -> getPosY()) <= MELEE_RADIUS * MELEE_RADIUS)
...
void Level::playGame(unsigned int time, unsigned int shotTime, unsigned 
                     int monsterTime){
